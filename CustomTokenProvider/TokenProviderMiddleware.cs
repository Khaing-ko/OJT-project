...
namespace TodoApi.CustomTokenAuthProvider
{
    public class TokenProviderMiddleware : IMiddleware
    {
            ...
            try
            {
                ...
                if (_loginType == "1")
                {
                    ...
                    if (loginresult.error == 0)
                    {
                        ...
                        AdminLevelname = loginresult.AdminLevelName;
                    }
                    else
                    {
        ...
        async Task<dynamic> DoAdminTypeloginValidation(string username, string password)
        {
            ...
            try
            {
                //var resultAdmin = await _repository.Admin.FindByConditionAsync(adm => adm.AdminLoginName == username);

                var resultAdmin = await _repository.Admin.GetAdminLoginValidation(username);
                ...
                bool flag = SaltedHash.Verify(objAdmin.Salt, objAdmin.adminPassword, password);

                if (flag == false || objAdmin.AdminStatus == false)  //incorrect pwd
                {
        ...
        public async Task<string> ReGenerateToken(HttpContext context)
        {
            try
            {
                ...
                if (allow)
                {
                    var userObj = await _repository.Admin.FindByIDAsync(Convert.ToInt64(_tokenData.UserID));
        ...
        async Task<bool> CheckURLPermission(TokenData obj, string ServiceURL)
        {
            try
            {
                var userlevelid = long.Parse(obj.UserLevelID);
