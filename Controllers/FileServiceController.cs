...
namespace TodoApi.Controllers
{
    ...
    public class FileServiceController : BaseController<FileServiceController>
    {
        ...
        public async Task<string> ProfilePhoto()  //to show current login user profile photo
        {
            ...
            try
            {
                string uploadDirectory = _configuration["appSettings:AdminPhoto"] ?? throw new Exception("Invalid AdminPhoto path.");
                ...
                {
                    Logger.LogWarning("Get ProfilePhoto File Path not found: {Folder}, {AdminID}", folderPath, AdminID);  //it is not error, but log to see someone trying random file path or not.
                    return "";
                }
            }
            catch (Exception ex)
            {
                Logger.LogError(ex, "Get ProfilePhoto fail AdminID: {ID}", AdminID);  //never return or output file path exception to users, just output to log file
        ...
        public async Task<string> DownLoadFile(string functionname, string param)  //restrict user level permission upto function name level
        {
            ...
            try
            {
                if (folderPath.Contains("..") || filename.Contains(".."))
        ...
        public async Task<TempFilePayload> FileUploadTempDir([FromForm] string tempdir, [FromForm] string enFile)
        {
            Response.ContentType = "application/json";
            try
            {
            ...
            try
            {
                ...
                if (files.Count > 0)
                {
                    ...
                    if (file.Length > 0)
                    {
                        ...
                        // fullPath = folderPath + uploadfilename;
        ...
        public string FileUploadRemoveDir(string functionname, string param, [FromBody] dynamic bodyParam) //restrict user level permission upto function name level
        {
            ...
            try
            {
                ...
                // string fileName = bodyParam["key"];

                if (bodyParam != null && bodyParam != "")
                {  //sub folder multiple file
                    string fileName = bodyParam;
        ...
        public string FileUploadTempRemove([FromForm] string fileNames)
        {
            ...
            try
            {
                ...
                if (fileNames == null || fileNames == "")
                    ...
                    uploadfilename = Encryption.DecryptFileName(fileNames);
                ...
                fullPath = baseDirectory + folderPath + uploadfilename;
                if (fullPath.Contains(".."))
                { //if found .. in the file name or path
                    Log.Error("Invalid path " + fullPath);
        ...
        public string FileUploadTempRemoveDir([FromForm] string fileNames, [FromForm] string tempdir, [FromForm] string tempfile)
        {
            ...
            try
            {
                ...
                if (tempfile == null || tempfile == "")
                    ...
                    encfileNames = Encryption.DecryptFileName(tempfile);

                Console.WriteLine(encfileNames);
